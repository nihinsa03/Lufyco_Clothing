...
const ColorDots = ({ colors, styles }: { colors: string[]; styles: any }) => {
  ...
  return (
    <View style={styles.colorRow}>
      ...
      {colors.length > 3 && <Text style={styles.moreColors}>+{colors.length - 3}</Text>}
    </View>
  );
};
  ...
  const fetchProducts = async () => {
    ...
    try {
      ...
      if (selectedFilter === "price_low_to_high") {
        ...
        data.sort((a, b) => (b.reviewsCount || b.reviews || 0) - (a.reviewsCount || a.reviews || 0));
      ...
      {loading ? (
        ...
        <FlatList
          ...
          renderItem={({ item }) => {
            ...
            const itemTitle = item.name || item.title || "Unknown Product";
            ...
            return (
              ...
              >
                ...
                <View style={styles.priceRow}>
                  <Text style={styles.price}>LKR {Number(item.price || 0).toFixed(2)}</Text>
                  {((item.compareAtPrice && item.compareAtPrice > item.price) ||
                    (item.oldPrice && item.oldPrice > item.price)) && (
                    <Text style={styles.compare}>
                      LKR {Number(item.compareAtPrice || item.oldPrice || 0).toFixed(2)}
                    </Text>
                  )}
                </View>
              </TouchableOpacity>
